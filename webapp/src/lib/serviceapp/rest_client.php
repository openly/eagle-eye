...
class RestClient {
    ...
    public function format_query($parameters, $dataformat='get', $primary='=', $secondary='&'){
        if($dataformat == 'json'){
    ...
    public function parse_response($response){
        $headers_size = curl_getinfo($this->handle, CURLINFO_HEADER_SIZE);
        $headers_content = substr($response, 0, $headers_size);       

        $http_ver[] = strtok($headers_content, "\n");
		...
		$this->response = substr($response, $headers_size);
    }
    ...
    public function execute($url, $method='GET', $parameters=array(), $headers=array()){
        ...
        if(strtoupper($method) == 'POST'){
            ...
            $curlopt[CURLOPT_POSTFIELDS] = $client->format_query($parameters,$client->options['data_format']);
        }
